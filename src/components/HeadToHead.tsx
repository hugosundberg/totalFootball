  ...
  return (
    <div className="flex flex-col bg-white dark:bg-zinc-900 w-full sm:w-11/12 justify-self-center max-w-[1200px] h-fit sm:rounded-2xl overflow-auto mt-5 mb-5">
      <div className="bg-white dark:bg-zinc-800">
        <div className="flex justify-between p-6 w-11/12 sm:w-3/4 lg:w-1/2 justify-self-center">
          <img src={fixture.teams.home.logo} alt="" className="h-10" />
          <div className="flex flex-col items-center gap-3">
            <div className="flex items-center bg-green-800 px-6 rounded-full p-2 text-white">
              {teamOneWins}
            </div>
            <p>Wins</p>
          </div>
          <div className="flex flex-col items-center gap-3">
            <div className="flex items-center bg-gray-800 px-6 rounded-full p-2 text-white">
              {draws}
            </div>
            <p>Draws</p>
          </div>
      ...
      {headToHead.map((match, index) => {
        const isLastMatch = index === headToHead.length - 1;
        return (
          <div
            key={index}
          ...
          >
            <div className="flex justify-between px-6 pt-4">
              ...
              <div className="flex gap-2 items-center text-sm dark:text-gray-400">
                ...
                <img
                  ...
                  className="h-8 dark:bg-gray-400 p-1 rounded-full"
                  alt="league-logo"
                />
              </div>
            </div>
            ...
            {!isLastMatch && (
              <span className="block w-full h-0.5 bg-zinc-300 dark:bg-zinc-800" />
            )}
          </div>
        );
      })}
